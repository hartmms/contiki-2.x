...
class renderpage(object):
   ...
   def index(self):
      ...
      yield '</form>'
      yield '<hr>'
      yield '<form name="time_form" action="cat_feeder" method="get" onSubmit="return checkSettings(document.time_form.time.value)">'
      ...
      yield '</form>'
      yield '<form name="feed_time_form" action="cat_feeder" method="get" onSubmit="return checkSettings(document.feed_time_form.feed_time.value)">'
      ...
      yield '</form>'
      yield '<hr>'
      yield '<form name="mac_addr_form" action="cat_feeder" method="get" onSubmit="checkMac();">'
      yield '    <p>MAC address'
   ...
   def header(self):
      ...
      header_text += 'event.returnValue=true;'
      header_text += 'else'
      header_text += 'event.returnValue=false;'
      header_text += '}'
      header_text += 'else'
      header_text += '{'
      ...
      header_text += 'function checkTime(time_from_form)'
      header_text += '{'
      header_text += 'var valid_time=/^[0-9]{1,2}:[0-9]{2}$/;'
      header_text += 'var valid_motor_time=/^[0-9]$/;'
      header_text += 'if (!valid_time.test(time_from_form))'
      header_text += '{'
      ...
      header_text += '{'
      header_text += 'if (!valid_motor_time.test(motor_time))'
      header_text += '{'
